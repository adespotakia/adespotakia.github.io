...
import { Calendar, User, MapPin, Heart, Clock } from "lucide-react";
...
const StraysRegistry = () => {
  ...
  const fetchStrays = async () => {
    try {
      ...
      // Fetch strays with their registerer information
  ...
  return (
    <>
      <div className="space-y-4">
        <div className="mb-6">
          <h2 className="text-xl font-semibold mb-2">Μητρώο Αδέσποτων</h2>
          <p className="text-gray-600 text-sm">
            Όλα τα αδέσποτα που έχουν καταχωρηθεί στην πλατφόρμα
          </p>
        </div>

        {strays.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">Δεν υπάρχουν καταχωρημένα αδέσποτα ακόμα</p>
          </div>
        ) : (
              ...
              <Card key={stray.id} className="overflow-hidden">
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {stray.age && (
                      <div>
                        <span className="font-medium">Ηλικία:</span> {stray.age} έτη
                      </div>
                    )}
                    {stray.gender && (
                      <div>
